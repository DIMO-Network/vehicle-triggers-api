...
func EventVehicles(mods ...qm.QueryMod) eventVehicleQuery {
	mods = append(mods, qm.From("\"vehicle_events_api\".\"event_vehicles\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"vehicle_events_api\".\"event_vehicles\".*"})
	}
...
func (o *EventVehicle) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"vehicle_events_api\".\"event_vehicles\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"vehicle_events_api\".\"event_vehicles\" %sDEFAULT VALUES%s"
		}
...
func (o *EventVehicle) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"vehicle_events_api\".\"event_vehicles\" SET %s WHERE %s",
...
func (o EventVehicleSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"vehicle_events_api\".\"event_vehicles\" SET %s WHERE %s",
...
func (o *EventVehicle) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"vehicle_events_api\".\"event_vehicles\"", updateOnConflict, ret, update, conflict, insert)
...
func (o EventVehicleSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"vehicle_events_api\".\"event_vehicles\" WHERE " +
...
func (o *EventVehicleSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"vehicle_events_api\".\"event_vehicles\".* FROM \"vehicle_events_api\".\"event_vehicles\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, eventVehiclePrimaryKeyColumns, len(*o))
