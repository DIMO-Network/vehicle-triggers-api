...
func (eventL) LoadDeveloperLicenseAddressDeveloperLicense(ctx context.Context, e boil.ContextExecutor, singular bool, maybeEvent interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		if !queries.IsNil(object.DeveloperLicenseAddress) {
			args = append(args, object.DeveloperLicenseAddress)
		}
	...
	Outer:
		for _, obj := range slice {
			...
			for _, a := range args {
				if queries.Equal(a, obj.DeveloperLicenseAddress) {
					continue Outer
				}
			}

			if !queries.IsNil(obj.DeveloperLicenseAddress) {
				args = append(args, obj.DeveloperLicenseAddress)
			}
	...
	query := NewQuery(
		...
		qm.WhereIn(`vehicle_events_api.developer_licenses.license_address in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (eventL) LoadEventLogs(ctx context.Context, e boil.ContextExecutor, singular bool, maybeEvent interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`vehicle_events_api.event_logs.event_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (eventL) LoadEventVehicles(ctx context.Context, e boil.ContextExecutor, singular bool, maybeEvent interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`vehicle_events_api.event_vehicles.event_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Event) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"vehicle_events_api\".\"events\"", updateOnConflict, ret, update, conflict, insert)
