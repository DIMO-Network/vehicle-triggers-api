...
var EventLogWhere = struct {
	...
	ID:               whereHelperstring{field: "\"event_logs\".\"id\""},
	VehicleTokenID:   whereHelpertypes_Decimal{field: "\"event_logs\".\"vehicle_token_id\""},
	EventID:          whereHelperstring{field: "\"event_logs\".\"event_id\""},
	SnapshotData:     whereHelpertypes_JSON{field: "\"event_logs\".\"snapshot_data\""},
	HTTPResponseCode: whereHelpernull_Int{field: "\"event_logs\".\"http_response_code\""},
	LastTriggeredAt:  whereHelpertime_Time{field: "\"event_logs\".\"last_triggered_at\""},
	EventType:        whereHelperstring{field: "\"event_logs\".\"event_type\""},
	PermissionStatus: whereHelperstring{field: "\"event_logs\".\"permission_status\""},
	CreatedAt:        whereHelpertime_Time{field: "\"event_logs\".\"created_at\""},
}
...
func (o *EventLog) SetEvent(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Event) error {
	...
	updateQuery := fmt.Sprintf(
		"UPDATE \"event_logs\" SET %s WHERE %s",
...
func EventLogs(mods ...qm.QueryMod) eventLogQuery {
	mods = append(mods, qm.From("\"event_logs\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"event_logs\".*"})
	}
...
func FindEventLog(ctx context.Context, exec boil.ContextExecutor, iD string, selectCols ...string) (*EventLog, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"event_logs\" where \"id\"=$1", sel,
	)
...
func (o *EventLog) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"event_logs\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"event_logs\" %sDEFAULT VALUES%s"
		}
...
func (o *EventLog) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"event_logs\" SET %s WHERE %s",
...
func (o EventLogSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"event_logs\" SET %s WHERE %s",
...
func (o *EventLog) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(eventLogAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(eventLogPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert event_logs, could not build conflict column list")
			}

			conflict = make([]string, len(eventLogPrimaryKeyColumns))
			copy(conflict, eventLogPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"event_logs\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o *EventLog) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"event_logs\" WHERE \"id\"=$1"
...
func (o EventLogSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"event_logs\" WHERE " +
...
func (o *EventLogSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"event_logs\".* FROM \"event_logs\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, eventLogPrimaryKeyColumns, len(*o))
...
func EventLogExists(ctx context.Context, exec boil.ContextExecutor, iD string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"event_logs\" where \"id\"=$1 limit 1)"
