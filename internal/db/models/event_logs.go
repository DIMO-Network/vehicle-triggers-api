...
func (o *EventLog) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"vehicle_events_api\".\"event_logs\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o EventLogSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"vehicle_events_api\".\"event_logs\" WHERE " +
		strmangle.WhereInClause(string(dialect.LQ), string(dialect.RQ), 1, eventLogPrimaryKeyColumns, len(o))
