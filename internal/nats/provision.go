	...
	streams := []jetstream.StreamConfig{
		{
			...
			MaxBytes:    c.cfg.SignalsMaxBytes,
		...
		},
		{
			...
			MaxBytes:    c.cfg.ConfigAuditMaxBytes,
			Replicas:    c.cfg.StreamReplicas,
...
// EnsureBuckets creates-or-updates the two KV buckets the service uses:
// trigger_state (per-trigger fire records driving cooldown and previousValue)
// and signal_history (per-vehicle per-metric fire records driving
// cross-trigger previousValue). Idempotent.
